        ...
        <div className={styles.grid} id="grid">
          {boxes.map((color, idx) => (
            <Box
              key={idx}              
              color={color}
              {... color === "snowWhite" ? {className: styles.glow, id: "glow"} : "" }
            />
          ))}
        </div>
      </div>
...
function isSolved(): boolean {    
  ...
  const glowColPos =  glow.getPropertyValue('grid-column-start') + glow.getPropertyValue('grid-column-end');
  const glowRowPos = glow.getPropertyValue('grid-row-start') + glow.getPropertyValue('grid-row-end');
  
  return (
    isGrid && 
    gridCols == 11 && gridRows == 10 && 
    pos.indexOf(glowColPos) > -1 && pos.indexOf(glowRowPos) > -1
  );  
}
